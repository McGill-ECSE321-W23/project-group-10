...
public class SingleReservationService extends ReservationService{
     ...
     */
    @Transactional
    public SingleReservation createSingleReservation(int reservationId, Date date, String licenseNumber, int parkingTime, ParkingSpot parkingSpot) {
        if (reservationId < 0){
     ...
     */
     ...
     public SingleReservation deleteReservation(int reservationId){
        if (reservationId < 0){
            throw new IllegalArgumentException("ReservationId cannot be negative.");
        }
        else if(reservationRepository.findReservationById(reservationId) == null){
     ...
     */
    @Transactional
    public Double calculateFee(Time startTime, SingleReservation singleReservation){
        Time nowTime = Time.valueOf(LocalTime.now());
        long parkingTime;
        parkingTime = nowTime.getTime() - startTime.getTime();
        Double fee = parkingTime * singleReservation.getParkingSpot().getType().getFee();
        return fee;
    }
